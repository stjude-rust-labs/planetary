...
pub async fn create_task(
    State(state): State<server::State>,
    ...
    state.orchestrator.start_task(&id);
...
pub async fn list_tasks(
    ...
    if page_size == 0 {
        return Err(bad_request("page size must be greater than zero"));
    }
...
pub async fn cancel_task(
    ...
    let id = id.strip_suffix(":cancel").ok_or_else(not_found)?;
    ...
    state.orchestrator.cancel_task(id);
