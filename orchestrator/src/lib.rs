...
mod tests {
    ...
    /// Build a test state with the given API key.
    async fn build_state_with_key(key: &str) -> Arc<State> {
        // Construct a minimal orchestrator instance; it won't be used by these tests.
        let db = Arc::new(
            planetary_db::postgres::PostgresDatabase::new(
                // A placeholder URL; no DB operations are performed in these tests.
                "postgresql://localhost/planetary_test".to_string().into(),
            )
            .expect("create db handle"),
        );
        ...
        Arc::new(State {
            service_api_key: key.to_string().into(),
            orchestrator,
        })
    }

    #[tokio::test]
    async fn bearer_token_valid_allows_request() {
        let state = build_state_with_key("secret").await;
        let inner = service_fn(|_req: axum::extract::Request| async move {
            Ok::<_, std::convert::Infallible>(Response::new(Body::empty()))
        });
        let svc = middleware::from_fn_with_state(state.clone(), auth).layer(inner);
        let req: axum::extract::Request = http::Request::builder()
            .uri("/v1/test")
    ...
    #[tokio::test]
    async fn bearer_token_missing_is_forbidden() {
        let state = build_state_with_key("secret").await;
        let inner = service_fn(|_req: axum::extract::Request| async move {
            Ok::<_, std::convert::Infallible>(Response::new(Body::empty()))
        });
    ...
    #[tokio::test]
    async fn bearer_token_invalid_is_forbidden() {
        let state = build_state_with_key("secret").await;
        let inner = service_fn(|_req: axum::extract::Request| async move {
            Ok::<_, std::convert::Infallible>(Response::new(Body::empty()))
        });
